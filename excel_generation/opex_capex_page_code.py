...
class OpexCapexPage:
    ...
    def populate_sheet(self):
        ...
        for expense in self.business_object.operating_expenses:
            ...
            # Combine amount and frequency notes
            combined_notes = f"Amount: {expense.get('amount_notes', '')}\nFrequency: {expense.get('frequency_notes', '')}"
            self.write_to_sheet(row, col + 4, combined_notes)
        ...
        for item in self.business_object.capex_items:
            logging.debug(f"Capital Expense: {item}")
            self.write_to_sheet(row, col, item['expense_name'])
            self.write_to_sheet(row, col + 1, item['amount'], format_name="currency_input")
            self.write_to_sheet(row, col + 2, item.get('depreciation_years', item.get('depreciation_life', 5)), format_name="input")  # Added fallback
            
            # Convert purchase_year to integer before using in datetime
            purchase_year = int(item['purchase_year'])
            self.write_to_sheet(row, col + 3, datetime(purchase_year, 1, 1), format_name="date_input_YYYY")
            
            # Combine amount and timing notes
            combined_notes = f"Amount: {item.get('amount_notes', '')}\nTiming: {item.get('timing_notes', '')}"
            self.write_to_sheet(row, col + 4, combined_notes)
            self.write_to_sheet(row, col + 5, " ")  # Prevent notes from running over
        ...
        for item in self.business_object.capex_items:
            logging.debug(f"Depreciation Item: {item}")
            self.write_to_sheet(row, col, item['expense_name'])
            self.write_to_sheet(row, col + 1, item['amount'], format_name="currency_input")
        ...
        for item in self.business_object.operating_expenses: 
            try:
                ...
                freq_source = item['frequency_source']
                
                source_text = (
                    f"Amount: {amount_source['display_value']} "
                    f"Frequency: {freq_source['display_value']}"
                )
                
                # If URLs are available and not default, create hyperlinks
                if amount_source['url'] != '-No Source-':
                    self.write_to_sheet(
                        row, col+1, 
                        f'=HYPERLINK("{amount_source["url"]}", "{source_text}")', 
                        format_name="URL_small"
                    )
                else:
                    self.write_to_sheet(row, col+1, source_text, format_name="small_italic")
                    
            except Exception as e:
                logging.debug(f"OPEX/CAPEX. Missing some table data: {str(e)}")
            self.operating_expenses_sheet.set_row(row, 10) #Make the row height smaller
            row+=1
        for item in self.business_object.capex_items:
            try:
                self.write_to_sheet(row, col, item['expense_name'], format_name='small_italic')
                ...
                # Handle nested source structure
                amount_source = item.get('amount_source', {'display_value': '-No Source-', 'url': '-No Source-'})
                timing_source = item.get('timing_source', {'display_value': '-No Source-', 'url': '-No Source-'})
                
                source_text = (
                    f"Amount: {amount_source['display_value']} "
                    f"Timing: {timing_source['display_value']}"
                )
                ...
                logging.debug(f"CAPEX Sources. Missing some table data: {str(e)}")
            self.operating_expenses_sheet.set_row(row, 10)  # Make the row height smaller
            row+=1